...
impl Server {
    ...
    pub async fn start(self: Arc<Self>) -> Result<()> {
        ...
        if Path::new(&self.SOCKET_PATH).exists() {
            fs::remove_file(&self.SOCKET_PATH).expect("[ERR]Could not remove file!!! "); //Mkae sure the file doesnt already exist
        }

        let listener = UnixListener::bind(&self.SOCKET_PATH).expect("[ERR]Failed to bind to socket!!! ");

        fs::set_permissions(&self.SOCKET_PATH, fs::Permissions::from_mode(0o700)).expect("[ERR]Failed to set the file permissions!!! ");
        ...
        loop {
            ...
            tokio::spawn(async move {
                self_clone.handle_client(stream, AUTH_STATE, &value.expose_secret()).await;
            });
    ...
    pub async fn handle_client(&self, mut stream: UnixStream, mut auth_state: bool, pass: &str) {
        let AUTH_TOKEN: &str = "SUPER-DUPER-SECRET!!!!";
